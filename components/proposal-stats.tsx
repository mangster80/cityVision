...
export function ProposalStats({ proposal }: { proposal: Proposal }) {
  ...
  return (
    <div className="my-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
      ...
      <div className="rounded-2xl bg-mint p-4"><MessageCircle size={18} className="mb-3 text-sage"/><p className="text-xl font-semibold">{proposal.comments}</p><p className="text-xs text-slate-500">kommentarer</p></div>
    </div>
  );
}
