...
export function ProposalGallery({ images, label, accent = false }: { images: string[]; label: string; accent?: boolean }) {
  ...
  return <>
    ...
    <button type="button" aria-label={`Öppna ${label.toLocaleLowerCase("sv-SE")}bilder`} onClick={() => setSelected(0)} className="group relative block h-[380px] w-full cursor-zoom-in overflow-hidden rounded-3xl text-left">
      ...
      <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/75 to-transparent px-4 pb-4 pt-10 text-left text-xs font-semibold text-white opacity-0 transition group-hover:opacity-100">{hasMultiple ? `Visa alla ${images.length} bilder` : "Öppna bild"}</span>
      {hasMultiple && <span className="absolute bottom-3 right-3 rounded-full bg-ink/75 px-3 py-1.5 text-xs font-semibold text-white backdrop-blur-md">{images.length} bilder</span>}
    </button>
