...
export function ProposalGallery({ images, label, accent = false }: { images: string[]; label: string; accent?: boolean }) {
  ...
  return <>
    ...
    <button type="button" aria-label={`${t("gallery.open")} ${label}`} onClick={() => setSelected(0)} className="group relative block h-[380px] w-full cursor-zoom-in overflow-hidden rounded-3xl text-left">
    ...
    {selected !== null && <div role="dialog" aria-modal="true" aria-label={`${label}, ${index + 1} / ${images.length}`} className="fixed inset-0 z-[9999] grid place-items-center bg-ink/85 p-5 backdrop-blur-sm" onClick={close}>
      <div className="relative h-[min(78vh,720px)] w-full max-w-5xl" onClick={event => event.stopPropagation()}>
        <Image sizes="100vw" src={images[index]} alt={`${label} (${index + 1})`} fill className="object-contain" />
        <button type="button" aria-label={t("gallery.close")} onClick={close} className="absolute right-0 top-0 grid h-11 w-11 place-items-center rounded-full bg-white/15 text-white hover:bg-white/25"><X size={20} /></button>
        {hasMultiple && <><button type="button" aria-label={t("gallery.previous")} onClick={() => setSelected((index - 1 + images.length) % images.length)} className="absolute left-0 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-white/15 text-white hover:bg-white/25"><ChevronLeft size={22} /></button><button type="button" aria-label={t("gallery.next")} onClick={() => setSelected((index + 1) % images.length)} className="absolute right-0 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-white/15 text-white hover:bg-white/25"><ChevronRight size={22} /></button><span className="absolute bottom-0 left-1/2 -translate-x-1/2 rounded-full bg-white/15 px-3 py-1.5 text-xs font-semibold text-white">{index + 1} / {images.length}</span></>}
      </div>
    </div>}
  </>;
}
