...
export default function CreatePage() {
  ...
  const handleImages = (event: ChangeEvent<HTMLInputElement>, setImages: (values: string[]) => void) => {
    ...
    }))).then(setImages).catch(() => setImageError(t("create.the-images-could-not-be-loaded")));
  };
  ...
  useEffect(() => {
    ...
    imageLabels.forEach((label, labelIndex) => {
      ...
      Array.from(imageGrid.querySelectorAll("img")).forEach((image, index) => {
        ...
        removeButton.setAttribute("aria-label", `${t("create.remove-image")} ${index + 1}`);
  ...
  const handleUseCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        setLocationStatus(`${t("create.location-selected")} ${coords.latitude.toFixed(4)}, ${coords.longitude.toFixed(4)}`);
        const locationInput = document.getElementById("location");
      ...
      () => setLocationStatus(t("create.could-not-get-your-location-check-location-permissions"))
    );
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!beforeImages.length || !afterImages.length) {
      setImageError(t("create.upload-at-least-one-before-image-and-one-after-image"));
      return;
    }
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : t("create.the-proposal-could-not-be-saved"));
      setIsSaving(false);
      return;
    }
