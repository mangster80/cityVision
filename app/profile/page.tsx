        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="-mt-12 flex min-w-0 flex-col items-start gap-4 sm:-mt-16 sm:flex-row sm:items-end">
            ...
            <div className="min-w-0 pb-1 sm:flex-1 sm:translate-y-4">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2"><h1 className="truncate text-2xl font-semibold leading-tight tracking-tight sm:text-4xl">{editing && isOwnProfile ? nameDraft : profileUser.name}</h1>{showAuthDetails && isVerified && <span title={t("Verifierad profil", "Verified profile")} aria-label={t("Verifierad profil", "Verified profile")} className="inline-flex items-center justify-center text-sage"><ShieldCheck size={19} strokeWidth={2.4}/></span>}</div>
              <p className="mt-1 truncate text-xs leading-5 text-slate-500 dark:text-slate-400 sm:text-sm">{[profileUser.city || "Stockholm", profileUser.neighborhood].filter(Boolean).join(" · ")}{showAuthDetails && memberSinceLabel ? ` · ${t("medlem sedan", "member since")} ${memberSinceLabel}` : ""}</p>
            </div>
          </div>
