...
function ProfileContent() {
  ...
  return <main className="px-5 pb-20 pt-28 sm:px-8 sm:pt-32"><div className="mx-auto max-w-6xl">
    ...
    {(profileUser.bio || profileUser.role || authEmail) && <section className="mt-5 grid gap-5 rounded-3xl border border-white/80 bg-white/75 p-6 shadow-sm backdrop-blur-xl sm:grid-cols-[1.4fr_1fr] sm:p-7 dark:border-white/10 dark:bg-[#201b35]/85"><div>{profileUser.bio ? <><p className="mb-2 text-xs font-bold uppercase tracking-[.16em] text-sage">{t("OM MIG", "ABOUT ME")}</p><p className="max-w-2xl text-sm leading-relaxed text-slate-500 dark:text-slate-300">{profileUser.bio}</p></> : <p className="text-sm text-slate-400">{t("Ingen presentation ännu.", "No introduction yet.")}</p>}</div><div className="border-t border-black/5 pt-5 text-sm text-slate-500 sm:border-l sm:border-t-0 sm:pl-6 sm:pt-0 dark:border-white/10 dark:text-slate-300">{profileUser.role && <p><span className="font-semibold text-ink dark:text-white">{t("Roll", "Role")}:</span> {profileUser.role}</p>}{profileUser.city && <p className="mt-2"><span className="font-semibold text-ink dark:text-white">{t("Ort", "City")}:</span> {profileUser.city}{profileUser.neighborhood ? ` · ${profileUser.neighborhood}` : ""}</p>}{authEmail && <><p className="mt-2"><span className="font-semibold text-ink dark:text-white">{t("Autentiserad via", "Authenticated via")}:</span> <span className="inline-flex items-center gap-1.5"><span className="inline-flex items-center justify-center">{authIcon}</span>{authLabel}</span></p><p className="mt-2 break-all text-xs text-slate-500 dark:text-slate-400">{authEmail}</p></>}</div></section>}
