  ...
  return <main className="px-5 pb-20 pt-32 sm:px-10"><div className="mx-auto max-w-7xl">
    ...
    <section className="rounded-[2rem] border border-black/5 bg-white shadow-sm dark:border-white/10 dark:bg-[#201b35]">
      ...
      <div className="relative z-10 -mt-12 min-h-[150px] px-6 pb-7 sm:-mt-16 sm:min-h-[170px] sm:px-10"><div className="absolute inset-x-6 bottom-7 flex flex-col items-stretch gap-4 sm:inset-x-10 sm:flex-row sm:items-end sm:justify-between sm:gap-5">
        <div className="flex min-w-0 items-end gap-3 sm:gap-4"><div className="relative h-20 w-20 shrink-0 sm:h-32 sm:w-32">{editing && isOwnProfile ? <label htmlFor="profile-avatar-file" className="group block h-full w-full cursor-pointer rounded-full"><span className="grid h-full w-full place-items-center overflow-hidden rounded-full border border-white/90 bg-mint p-1.5 shadow-[0_8px_24px_rgba(23,19,38,.22)] dark:border-[#201b35] dark:bg-[#292044]"><Image src={avatarPreview || profileUser.avatar} alt={`Profilbild för ${nameDraft}`} width={320} height={320} className="h-full w-full rounded-full object-cover transition group-hover:brightness-75"/></span><span className="absolute bottom-0 right-0 grid h-8 w-8 place-items-center rounded-full border-2 border-white bg-ink text-white shadow-md dark:border-[#201b35]"><Pencil size={14}/></span></label> : <span className="grid h-full w-full place-items-center overflow-hidden rounded-full border border-white/90 bg-mint p-1.5 shadow-[0_8px_24px_rgba(23,19,38,.22)] dark:border-[#201b35] dark:bg-[#292044]"><Image src={profileUser.avatar} alt={`Profilbild för ${profileUser.name}`} width={320} height={320} className="h-full w-full rounded-full object-cover"/></span>}</div><div className="min-w-0 self-end pb-1"><span className="inline-flex max-w-full items-center overflow-hidden text-ellipsis whitespace-nowrap rounded-full border border-sage/30 bg-mint px-2.5 py-1 text-[9px] font-bold tracking-[.1em] text-sage sm:px-3 sm:text-[10px] sm:tracking-[.14em] dark:bg-[#292044]">STADSLYFT-MEDLEM</span><div className="mt-2 flex items-center gap-2"><h1 className="truncate text-2xl font-semibold leading-tight tracking-tight sm:text-4xl">{editing && isOwnProfile ? nameDraft : profileUser.name}</h1>{authEmail && <span className="inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-white/80 px-2 py-1 text-[10px] font-semibold text-slate-600 dark:border-white/10 dark:bg-[#292044] dark:text-slate-200"><span className="inline-flex items-center justify-center">{authIcon}</span>{authLabel}</span>}</div><p className="truncate text-xs leading-5 text-slate-500 dark:text-slate-400 sm:text-sm">{[profileUser.city || "Stockholm", profileUser.neighborhood].filter(Boolean).join(" · ")} · med sedan september 2024</p></div></div>
    ...
    {(profileUser.bio || profileUser.role || authEmail) && <section className="mt-5 rounded-3xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#201b35]"><div className="grid gap-5 sm:grid-cols-[1.4fr_1fr]"><div>{profileUser.bio && <><p className="mb-2 text-xs font-bold uppercase tracking-[.16em] text-sage">{t("OM MIG", "ABOUT ME")}</p><p className="text-sm leading-relaxed text-slate-500 dark:text-slate-300">{profileUser.bio}</p></>}</div><div className="text-sm text-slate-500 dark:text-slate-300">{profileUser.role && <p><span className="font-semibold text-ink dark:text-white">{t("Roll", "Role")}:</span> {profileUser.role}</p>}{profileUser.city && <p className="mt-2"><span className="font-semibold text-ink dark:text-white">{t("Ort", "City")}:</span> {profileUser.city}{profileUser.neighborhood ? ` · ${profileUser.neighborhood}` : ""}</p>}{authEmail && <p className="mt-2"><span className="font-semibold text-ink dark:text-white">{t("Autentiserad via", "Authenticated via")}:</span> <span className="inline-flex items-center gap-1.5"><span className="inline-flex items-center justify-center">{authIcon}</span>{authLabel}</span></p>}{authEmail && <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">{authEmail}</p>}</div></div></section>}
