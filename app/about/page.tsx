...
export default function AboutPage() {
  const { t } = useLanguage();
  const steps = [
    { number: "01", icon: MapPin, iconColor: "text-blue-500 dark:text-blue-400", title: t("about.step1Title"), text: t("about.step1Text") },
    { number: "02", icon: Lightbulb, iconColor: "text-amber-500 dark:text-amber-400", title: t("about.step2Title"), text: t("about.step2Text") },
    { number: "03", icon: Flame, iconColor: "text-orange-500 dark:text-orange-400", title: t("about.step3Title"), text: t("about.step3Text") },
    { number: "04", icon: Clock, iconColor: "text-emerald-500 dark:text-emerald-400", title: t("about.step4Title"), text: t("about.step4Text") }
  ];
  return (
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="grid gap-10 py-20 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-3xl bg-mint p-7 dark:bg-[#292044]"><Compass className="mb-8 text-blue-500 dark:text-blue-400" size={25} /><h3 className="text-xl font-semibold">{t("about.potential")}</h3><p className="mt-3 text-sm leading-relaxed text-slate-500">{t("about.potentialText")}</p></div>
            <div className="rounded-3xl border border-black/10 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-[#201b35]"><Flame className="mb-8 text-orange-500 dark:text-orange-400" size={25} /><h3 className="text-xl font-semibold">{t("about.matters")}</h3><p className="mt-3 text-sm leading-relaxed text-slate-500">{t("about.mattersText")}</p></div>
            <div className="rounded-3xl border border-black/10 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-[#201b35]"><Sparkles className="mb-8 text-purple-600 dark:text-purple-400" size={25} /><h3 className="text-xl font-semibold">{t("about.creators")}</h3><p className="mt-3 text-sm leading-relaxed text-slate-500">{t("about.creatorsText")}</p></div>
          </div>
        </section>
        ...
        <section className="rounded-[2rem] border border-black/10 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-[#201b35] sm:p-10">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{steps.map(({ number, icon: Icon, iconColor, title, text }) => <div key={number} className="relative rounded-2xl border border-black/5 bg-slate-50/50 p-6 dark:border-white/5 dark:bg-white/[0.02]"><span className="text-sm font-bold text-[#7056d8]">{number}</span><Icon className={`mt-5 ${iconColor}`} size={24} /><h3 className="mt-4 text-lg font-semibold">{title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-500">{text}</p></div>)}</div>
        </section>
