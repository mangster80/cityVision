      ...
      <div className="mx-auto max-w-6xl">
        <section className="relative overflow-hidden rounded-[2.5rem] bg-ink px-7 py-14 text-white shadow-2xl sm:px-14 sm:py-20">
          ...
          <div className="relative max-w-3xl">
            <p className="mb-5 text-xs font-bold uppercase tracking-[.2em] text-[#b9a9ff]">OM STADSLYFT</p>
            ...
            <p className="mt-7 max-w-2xl text-lg leading-relaxed text-white/70 sm:text-xl">
              Stadslyft gör det enkelt att gå från frustration till förslag. Här kan invånare och lokala konstnärer dela idéer om våra gemensamma platser och göra dem synliga, konkreta och möjliga att agera på.
            </p>
            <div className="mt-9 flex flex-wrap gap-3">
              <Link href="/explore" className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3.5 text-sm font-semibold text-ink transition hover:bg-[#e8e1ff] dark:bg-mint dark:text-ink dark:hover:bg-white">Utforska platser <ArrowRight size={17} /></Link>
              <Link href="/create" className="inline-flex items-center gap-2 rounded-full border border-white/50 px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-white/15 dark:border-white/70 dark:hover:bg-white/20">Dela en idé <Sparkles size={17} /></Link>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-10 py-20 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div>
            <p className="mb-4 text-xs font-bold uppercase tracking-[.18em] text-sage">VARFÖR STADSLYFT?</p>
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">Små förändringar kan göra stor skillnad.</h2>
            <p className="mt-5 leading-relaxed text-slate-500">En otrygg passage, ett tomt torg eller en grå betongvägg påverkar vardagen mer än vi kanske tänker på. Stadslyft samlar lokalkännedom och kreativitet där den hör hemma: hos människorna som använder platserna – och hos lokala konstnärer som kan ge dem nytt liv.</p>
          </div>
        ...
        <section className="rounded-[2rem] border border-black/10 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-[#201b35] sm:p-10">
          <div className="max-w-2xl"><p className="mb-4 text-xs font-bold uppercase tracking-[.18em] text-sage">SÅ FUNGERAR DET</p><h2 className="text-3xl font-semibold tracking-tight">Från plats till möjlig förändring.</h2><p className="mt-4 text-slate-500">Stadslyft är byggt för att göra det lätt att delta, oavsett om du har en färdig idé eller bara ser något som kan bli bättre.</p></div>
        ...
        <section className="rounded-[2rem] bg-mint px-7 py-12 text-center dark:bg-[#292044] sm:px-10">
          ...
          <Link href="/explore" className="mt-8 inline-flex items-center gap-2 rounded-full bg-ink px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-sage">Utforska Stadslyft <ArrowRight size={17} /></Link>
        </section>
      </div>
    </main>
  );
